    ...
    <main className="bg-black flex min-h-screen flex-col items-center justify-center ">
      <div className="h-[28rem] p-5 w-2/3 bg-cyan-600 rounded-lg flex">
        ...
        <div className="flex-[3] px-16 min-h-full flex flex-col text-white justify-center items-center gap-5">
          ...
          <input value={name} onChange={e => setName(e.target.value)} className="w-full p-2 rounded text-black" placeholder="Digite seu nome" type="text" />
          <input value={email} onChange={e => setEmail(e.target.value)} className="w-full p-2 rounded text-black" placeholder="Digite seu email" type="email" />
          <button onClick={onSubmit} className="bg-indigo-500 p-4 rounded min-w-full">Inscreva-se</button>
        </div>
      </div>
    </main>
  );
